...
<div class="container-fluid">
	<div class="row">
        ...
        @if(Session::has('message'))
			...
			@endif
			<h3>Edit Category</h3>
			<form method="post" action="{{url('editcategory')}}/{{$singledata->cid}}" >
				{{ csrf_field() }}
				<input type="hidden" name= "tbl" value="{{encrypt('categories')}}">
				<input type="hidden" name="cid" value="{{$singledata->cid}}">
                <div class="form-group">
					<label>Name</label>
					<input type="text" name="category_name" id="category_name" class="form-control" value="{{$singledata->category_name}}" > 
					...
					<tbody>
						...
						<tr>
							<td>
								...
								<a href="{{url('editcategory')}}/{{$category->cid}}">{{$category->category_name}}</a>
							</td>
							<td>{{$category->slug}}</td>
